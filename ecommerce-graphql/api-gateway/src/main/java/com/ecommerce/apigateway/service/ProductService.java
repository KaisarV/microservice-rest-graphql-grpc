...
public class ProductService {
    ...
    public Product saveProduct(ProductRequest productRequest) {
        String mutation = String.format(
                "mutation { addProduct(productRequest: { id: %s, productCode: \\\"%s\\\", productTitle: \\\"%s\\\", imageUrl: \\\"%s\\\", discountOffer: %f, price: %f }) { id, productCode, productTitle, imageUrl, discountOffer, price, currentPrice }}",
    ...
    public List<Product> getAllProducts() {
        String query = "query { allProducts { id, productCode, productTitle, imageUrl, discountOffer, price, currentPrice }}";

        return executeGraphQLQuery(query, "allProducts", new TypeReference<List<Product>>() {});
    }

    public Product getProductById(Integer productId) {
        String query = String.format("query { productById(id: %d) { id, productCode, productTitle, imageUrl, discountOffer, price, currentPrice }}", productId);
    ...
    public Product updatePrice(Integer productId, Double price) {
        String mutation = String.format(
                "mutation { addPrice(addPriceRequest: { id: %d, price: %f }) { id, productCode, productTitle, imageUrl, discountOffer, price, currentPrice }}",
                productId, price
        );
