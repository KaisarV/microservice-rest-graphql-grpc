...
public class OfferService {
    ...
    public Product addProductOffer(OfferRequest offerRequest) {
        ...
        String productQuery = String.format("{\"query\": \"query {" +
                " productById(id: %d) { " +
                "productCode, productTitle, imageUrl, price}}\"}", offerRequest.getProductId());
        HttpEntity<String> productRequest = new HttpEntity<>(productQuery, headers);

        try {
            ResponseEntity<String> productResponse = restTemplate.exchange(GRAPHQL_URL, HttpMethod.POST,
